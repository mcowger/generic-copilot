...
suite("Utils Test Suite", () => {
	...
	suite("convertMessages", () => {
		...
		test("should convert multiple text parts", () => {
			const messages = [
				{
					role: vscode.LanguageModelChatMessageRole.User,
					content: [new vscode.LanguageModelTextPart("Part 1"), new vscode.LanguageModelTextPart("Part 2")],
					name: "",
				},
			];
		...
		test("should convert tool call message", () => {
			const messages = [
				{
					role: vscode.LanguageModelChatMessageRole.Assistant,
					content: [new vscode.LanguageModelToolCallPart("call_123", "get_weather", { location: "NYC" })],
					name: "",
				},
			];
			...
			const messages = [
				{
					role: vscode.LanguageModelChatMessageRole.User,
					content: [
						new vscode.LanguageModelToolResultPart("call_123", [new vscode.LanguageModelTextPart("Sunny, 72°F")]),
					],
					name: "",
				},
			];
	...
	suite("validateTools", () => {
		...
		test("should throw for invalid tool names", async () => {
			const tools = [{ name: "invalid name!", description: "", inputSchema: {} }];

			await assertThrowsAsync(async () => validateTools(tools), "Invalid tool name");
		});
	});
	...
	suite("validateRequest", () => {
		...
		test("should throw for empty messages", async () => {
			await assertThrowsAsync(async () => validateRequest([]), "Invalid request");
		});
		...
		test("should validate tool call followed by tool result", () => {
			const messages = [
				{
					role: vscode.LanguageModelChatMessageRole.Assistant,
					content: [new vscode.LanguageModelToolCallPart("call_1", "tool", {})],
					name: "",
				},
				{
					role: vscode.LanguageModelChatMessageRole.User,
					content: [new vscode.LanguageModelToolResultPart("call_1", [new vscode.LanguageModelTextPart("result")])],
					name: "",
				},
			];
		...
		test("should throw when tool call not followed by tool result", async () => {
			const messages = [
				...
				{
					role: vscode.LanguageModelChatMessageRole.User,
					content: [new vscode.LanguageModelTextPart("Just text")],
					name: "",
				},
			];

			await assertThrowsAsync(async () => validateRequest(messages), "Tool call part must be followed");
		});
	});
	...
	suite("executeWithRetry", () => {
		...
		test("should not retry on non-retryable error", async () => {
			...
			await assertThrowsAsync(async () => await executeWithRetry(fn, config, token), "400");
			assert.strictEqual(callCount, 1);
		});
		...
		test("should respect max_attempts", async () => {
			...
			await assertThrowsAsync(async () => await executeWithRetry(fn, config, token), "500");
			assert.strictEqual(callCount, 2);
		});
		...
		test("should handle cancellation during retry", async () => {
			...
			await assertThrowsAsync(async () => await executeWithRetry(fn, config, token), /cancelled|Service Unavailable/);
		});
		...
		test("should not retry when disabled", async () => {
			...
			await assertThrowsAsync(async () => await executeWithRetry(fn, config, token), "429");
			assert.strictEqual(callCount, 1);
		});
		...
		test("should retry on all retryable status codes", async () => {
			...
			for (const status of retryableStatuses) {
				...
				await assertThrowsAsync(async () => await executeWithRetry(fn, config, token), String(status));
			}
		});
	});
});
