	...
	<script nonce="${nonce}">
						...
						\${provider.defaults ? \`
							<div class="collapsible-content">
								...
								<div class="form-group">
									...
									<input type="number" step="0.1" value="\${provider.defaults.temperature ?? ''}" 
										onchange="updateProviderDefault(\${index}, 'temperature', parseFloat(this.value))">
								</div>
								<div class="form-group">
									<label>Top P (0-1)</label>
									<input type="number" step="0.1" value="\${provider.defaults.top_p ?? ''}" 
								...
								<div class="form-group">
									<label class="checkbox-label">
										<input type="checkbox" 
											\${provider.defaults.vision ? 'checked' : ''}
			...
			container.innerHTML = models.map((model, index) => {
				return \`
					<div class="item">
						...
						<div class="form-group">
							<label>Temperature (0-2)</label>
							<input type="number" step="0.1" value="\${model.temperature ?? ''}" 
								onchange="updateModel(\${index}, 'temperature', this.value === '' ? null : parseFloat(this.value))">
						</div>
						<div class="form-group">
							<label>Top P (0-1)</label>
							<input type="number" step="0.1" value="\${model.top_p ?? ''}" 
		...
		function saveConfiguration() {
			...
			if (invalidProviders.length > 0) {
				vscode.postMessage({
					command: 'error',
					message: 'Please fill in required fields (key and baseUrl) for all providers.'
				});
				return;
			}
			...
			if (invalidModels.length > 0) {
				vscode.postMessage({
					command: 'error',
					message: 'Please fill in required field (id) for all models.'
				});
				return;
			}
